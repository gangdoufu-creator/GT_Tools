...
class CopyAnimBetweenRigsUI:
    ...
    def show(self):
        ...
        self.set_namespaces_from_selection()
    ...
    def build_ui(self):
        ...
        self.src_sel_btn = cmds.button(label='<<', width=30, command=self.set_namespaces_from_selection)
        cmds.text(label='Target Namespace:')
        self.target_ns = cmds.textField(text='No_Tail_Spine_Rig:')
        self.tgt_sel_btn = cmds.button(label='<<', width=30, command=self.set_namespaces_from_selection)
        cmds.setParent('..')
    ...
    def set_namespaces_from_selection(self, *args):
        sel = cmds.ls(selection=True, long=True)
        if len(sel) >= 2:
            # First selected = source, second selected = target
            # Remove DAG path (everything before the last |) before extracting namespace
            src_clean = sel[0].split('|')[-1]
            tgt_clean = sel[1].split('|')[-1]
            
            src_parts = src_clean.split(':')
            tgt_parts = tgt_clean.split(':')
            
            if len(src_parts) > 1:
                src_ns = ':'.join(src_parts[:-1]) + ':'
                cmds.textField(self.source_ns, edit=True, text=src_ns)
            
            if len(tgt_parts) > 1:
                tgt_ns = ':'.join(tgt_parts[:-1]) + ':'
                cmds.textField(self.target_ns, edit=True, text=tgt_ns)
        elif len(sel) == 1:
            # Only one selected, use it for source
            # Remove DAG path before extracting namespace
            clean_name = sel[0].split('|')[-1]
    ...
    def add_selection_to_controls(self, *args):
        ...
        if sel:
            ...
            # Replace the field content (don't merge)
            cmds.textField(self.controls_field, edit=True, text=', '.join(names))
    ...
    def copy_anim(self, *args):
        ...
        if controls_text.strip():
            ...
            if cmds.objExists(control_set_name) and cmds.objectType(control_set_name) == 'objectSet':
                ...
                # Extract short names without namespace - strip any DAG path separators
                control_list = []
                for obj in set_members:
                    # Remove namespace prefix, keep only the last part after colon
                    if ':' in obj:
                        short_name = obj.split(':')[-1]
                    else:
                        short_name = obj
                    # Remove any DAG path separators (pipes)
                    short_name = short_name.split('|')[-1]
                    control_list.append(short_name)
                ...
                control_list = []
                for ctrl in all_source:
                    # Remove namespace and DAG path separators
                    if ':' in ctrl:
        ...
        for ctrl in control_list:
            # Clean control name thoroughly - remove all DAG path separators and whitespace
            ctrl_clean = ctrl.strip()
            # Remove any leading pipes
            while ctrl_clean.startswith('|'):
                ctrl_clean = ctrl_clean[1:]
            # Also split by pipe and take last part to ensure no hierarchy
            if '|' in ctrl_clean:
                ctrl_clean = ctrl_clean.split('|')[-1]
